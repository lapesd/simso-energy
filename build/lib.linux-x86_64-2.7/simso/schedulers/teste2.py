...
@scheduler("simso.schedulers.teste2")
class teste2(Scheduler):
    """
...
class ProperSubsystem(object):
    ...
    def end_dummy(self, cpu):
        ...
        #self.sim.logger.log("end_dummy dual {}".format(cpu.name))
    ...
    def CSC(self, t):
        ...
        active_servers = [s for s in self.root.children if (s.dummyServer is False) and s.budget > 0]
        ...
        if self.is_idle is True:
            #self.sim.logger.log("was IDLE before t - {}".format(self.processors[0].name))
            
            delta = delta_t(self, servers, t, t)
            #self.sim.logger.log("delta = {}".format(delta))
            ...
            if delta > 0:
                ...
                #self.sim.logger.log("u = {}, 1-u = {}".format(self.utilization, float(1-self.utilization)))
            ...
            if active_servers:
                ...
                delta = delta_t(self, servers,t, self.root.next_deadline)
                #self.sim.logger.log("delta = {}".format(delta))
                ...
                self.idleEnd = self.root.next_deadline + delta
                self.busyBegin = self.idleEnd
                self.busyEnd = self.idleBegin + (self.idleEnd - self.idleBegin)/ float(1-self.utilization)
                #self.keep = True
    ...
    def schedule(self):
        ...
        if idle:
            ...
            # CSC primal level
                ...
                #self.sim.logger.log("active low-power state during [{},{})".format(self.idleBegin, self.idleEnd))
