...
from simso.core.Energy import Energy
...
class Model(Simulation):
    ...
    def __init__(self, configuration, callback=None):
        ...
        for proc_info in proc_info_list:
            # Add power consumption to Processor
            energy = Energy(self)
            proc = Processor(self, proc_info, energy)

            proc.caches = proc_info.caches

            self._processors.append(proc)
            self._power.append(energy)
    ...
    def units(self):
        """
        List of all the power consumptions.
        """
        return self._power
