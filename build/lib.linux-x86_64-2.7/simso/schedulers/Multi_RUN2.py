...
@scheduler("simso.schedulers.Multi_RUN2")
class Multi_RUN2(Scheduler):
    """
...
class ProperSubsystem(object):
    ...
    def dummy_activated(self, cpu):
        ...
        update_deadline(self.sim, self.root, self.dummyTask[0])
        self.is_idle = False
        self.keep = False
    ...
    def schedule(self):
        ...
        active_servers = [s for s in self.root.children if s.budget > 0 and not s.dummyServer]
        ...
        if idle:
            #self.sim.logger.log("DUMMY TASK Executing on {}".format(self.processors[0].name)) 
            processors = self.processors[1:]
            decision.append((None, self.processors[0]))
            if self.idle < self.sim.now():
                time = self.root.next_deadline - self.sim.now()
                self.processors[0].set_idle(time)
                self.idle = self.root.next_deadline
