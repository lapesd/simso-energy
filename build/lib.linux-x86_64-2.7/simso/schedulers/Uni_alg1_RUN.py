...
from simso.schedulers.Uni_RUNServer import EDFServer, TaskServer, DualServer, \
    ...
    update_deadline2, uni_algorithm_1, algorithm_2, release_dummy_job, \
    next_activation
from simso.schedulers.Multi_RUNServer import algorithm_1
...
@scheduler("simso.schedulers.Uni_alg1_RUN")
class Uni_alg1_RUN(Scheduler):
    """
...
class ProperSubsystem(object):
    ...
    def __init__(self, sim, root, processors, dummy, u):
        self.sim = sim
        self.root = root
        ...
        self.sim.logger.log("u= {} or u = {}".format(self.utilization, float(self.utilization)))
    ...
    def schedule(self):
        ...
        """if (idle and not self.keep):
            ...
            if start_slack > 10:
                ...
                self.sim.logger.log("slack {} D {}".format(self.slack, start_slack))

                self.slack += start_slack

                
                if int(self.slack) > 0 and self.utilization < 0.995:
                    self.sim.logger.log("add_timer {} D {}".format(self.slack, start_slack))
                    self.add_timer(self.slack, self.processors[0])
                    self.keep = True
                else:
                    update_deadline2(self.sim, self.root, self.dummyTask[0], INFINITO)
