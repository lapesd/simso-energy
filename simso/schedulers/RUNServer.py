...
def update_deadline2(sim, root, server):

    last = None
    while server != root:
        server.deadlines = []
        server.next_deadline = INFINITO
...
def deadline(self, server, t):
    ...
    if server.task:
        ...
        child = [child for child in server.child.children if not child.dummyServer]
        min_server = min(child, key=lambda sv: sv.next_deadline)

        new_deadline = [d for d in server.deadlines if d > t]

        new_deadline.append(min_server.next_deadline + (min_server.task.period * self.sim.cycles_per_ms))

        """if server.next_deadline > t:
            return server.next_deadline"""

        return min(new_deadline)
...
def algorithm_1(self, servers, t):
    ...
    for j in range(0, len(servers)):
        ...
        for i in range(0, j+1):
            ...
            di = R(self, servers[i], d, t)
            #self.sim.logger.log("i = {}, d = {}, ri = {}, di = {}".format(servers[i].identifier, d, ri, di))

            if ri > t:
                ci = servers[i].budget#budget(self, servers[i], t) 
            else:
                ci = 0

            """if di < ri:
                di = ri = 0"""
            c = c + servers[i].budget + float(di - ri)*servers[i].utilization
            #self.sim.logger.log("c = {}, ci = {}, eita = {}, u = {}".format(c, servers[i].budget, float(di - ri)*servers[i].utilization, servers[i].utilization))
            #self.sim.logger.log("---------".format())
...
def R(self, server, d, t):
    ...
    if d < server.next_deadline:
        ...
        if d == deadline(self, server, t):
            return d
        elif d > deadline(self, server, t):
            next_d = deadline(self, server, t)
            period = deadline(self, server, t)-release(self, server, t)
            last_r = server.next_deadline
...
def algorithm_2(self, servers, t):
    ...
    slack = (D_t - t) - acumulated

    self.sim.logger.log("acumulated: {}".format(slack))

    """if delta < slack:
        delta = slack"""


    return int(delta) #+ (self.root.next_deadline-t)
...
def select_jobs(self, server, virtual, execute=True):
    ...
    if server.task:
        ...
        if execute and server.budget > 0:
            ...
            self.sim.logger.log("Selecionou servidor {} sem budget".format(server.identifier))
        ...
        # Rule 1
            ...
            if active_servers:
                ...
                if self.keep and server == self.root:
